...
| `data/eval_results/` | Detailed per-patient evaluation outputs (extractor / query / retriever / scorer metrics) |
| Audit & comparison reports | Per-patient HTML / CSV reports generated by the pipeline (e.g. files written to `exports/`) |

Requests for access can be sent to the author at **anassheikh57@gmail.com**. Requests are reviewed on a case-by-case basis and are subject to any applicable data-sharing restrictions, ethical approvals, and institutional agreements.
